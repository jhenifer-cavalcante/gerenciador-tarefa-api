...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.enum('status', Object.values(Status), {useNative: true, enumName: 'status_enum'}).defaultTo(Status[0]);
      table.string('descricao');
      table.date('prazo');
      table.enum('frequencia', Object.values(Frequencia), {useNative: true, enumName: 'frequencia_enum'}).defaultTo(Frequencia[5]);
      ...
      table.integer('num_recompensa').unsigned().references('numero').inTable('recompensa').onDelete('SET NULL');
